	...
	if(Key == KEY_ENTER)
	{
		...
		if(StrProc->CheckSNFormat(edtBarcodeValue->Text,false))
		{
			...
			if(!bl)
			{
				...
				edtBarcodeValue->Enabled = false;
				StrProc->SetSNFormat("XXXXXXXXX_____X");
				ShowWorkOrderForm();
		...
		{
			CheckPackingStatus();
			if(packingStatus[(plNo1->Caption.Delete(1,8).ToInt())-1].iStatus != PACKING_STATUS_COMPLETE)
			{
	...
	if(Key == KEY_ENTER)
	{
		...
		else if(edt->Name.Pos("edtModelNo"))
		{
				...
				&& StrProc->CheckStringNotNumber(SS.SubString(1,SS.Pos("-")-1)))
			{
				if(edt->Name.Pos("1"))
				{
					EnabledPackingInfoPanel(pledtQTY1,pledtModelNo1);
					edtQTY1->SetFocus();
		...
		else if(edt->Name.Pos("edtQTY1"))
		{
			if(edt->Text.Pos(plPcsCtn->Caption))
			{
				EnabledPackingInfoPanel(pledtSettingSNStart1,pledtQTY1);
				edtSettingSNStart1->SetFocus();
			}
			else
			{   //輸入確認格式及是否在範圍內
		...
		{   //確認輸入是否為數字且數值不超過3位數 true : 自動填入上下限及數字欄位
			if(edtQTY1->Text != "")
			{
				if(edt->Name.Pos("1"))
				{
					if(CheckCartonSNRange(edtSettingSNStart1->Text,edtSettingSNStart1,edtSettingSNLast1,plNo1))
					{
				...
				{
					/*if(CheckCartonSNRange(edtSettingSNStart2->Text,edtSettingSNStart2,edtSettingSNLast2,plNo2))
					{
...
bool TfrmMain::CheckCartonSNRange(AnsiString KeyIn,TEdit* edtStart,TEdit* edtLast,TPanel* plNo)
{
	...
	int PackingMaxNum = plPcsCtn->Caption.ToInt();
	int iKeyQTY =	plPcsCtn->Caption.ToInt();
	AnsiString strTemp = "";
	...
	if(StrProc->CheckStringNumber(LastCode) && StrProc->CheckStringNumber(StartCode))
	{
		...
		if(StartCode.ToInt() <= NowCode.ToInt() && NowCode.ToInt() <= LastCode.ToInt())
		{
			...
			else
			{
				...
				if(QuotientTemp % (PackingMaxNum) == 0)
				{
					plNo->Caption = "C / NO : "+AnsiString(QuotientTemp / (PackingMaxNum) );
					dwCartonNo = QuotientTemp / (PackingMaxNum);
				}
				else
				{
					plNo->Caption = "C / NO : "+AnsiString((QuotientTemp / (PackingMaxNum) )+1);
				}
			}
			//
...
DWORD TfrmMain::ReworkCheckBacodeValueToGrid(AnsiString BarcodValue)
{
	...
	for(int i = 0 ; i < iTotalCTNCount ; i++)
	{
		if(StrProc->GetSNNumber(plStartCode->Caption)+(100*i) <= StrProc->GetSNNumber(BarcodValue)
			&&	StrProc->GetSNNumber(plStartCode->Caption)+(100*i)+100-1 >= StrProc->GetSNNumber(BarcodValue))
		{
			iKeyInC_NO = i+1;
			edtSettingSNStart1->Text = StrProc->GetSNString(StrProc->GetSNNumber(plStartCode->Caption)+(100*i));
			if(StrProc->GetSNNumber(plStartCode->Caption)+(100*i)+99 < StrProc->GetSNNumber(plLastCode->Caption))
			{
				edtSettingSNLast1->Text = StrProc->GetSNString(StrProc->GetSNNumber(plStartCode->Caption)+(100*i)+99);
				edtSettingSNStart2->Text = StrProc->GetSNString(StrProc->GetSNNumber(plStartCode->Caption)+(100*i)+100);
				if(StrProc->GetSNNumber(plStartCode->Caption)+(100*i)+99 < StrProc->GetSNNumber(plLastCode->Caption))
					edtSettingSNLast2->Text = StrProc->GetSNString(StrProc->GetSNNumber(plStartCode->Caption)+(100*i)+99);
				else
...
{
	frmPackingStatus = NULL;
	delete frmPackingStatus;
}
//---------------------------------------------------------------------------
